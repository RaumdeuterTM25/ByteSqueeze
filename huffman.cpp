...
// DONE, this is called along with the first constructor call 
...
void huffman::traverse(Node* r, string str) {
    if (r->left == NULL && r->right == NULL) {
        r->code = str;  // storing the code at the 'code' member variable of each leaf node
        return;
    }
...
string huffman::decToBin(int inNum) {
    string temp = "", res = "";
    while (inNum > 0) {
        temp += (inNum % 2 + '0'); // converts the least significant bit to its corresponding ASCII character
        inNum /= 2;
    }
    
    res.append(8 - temp.length(), '0');  // to append leading zeros to make the string size no less than 8 

    for (int i = temp.length() - 1; i >= 0; i--) { 
        res += temp[i];                              // temp string is then added back to zero appended string 
    }
    return res;
}
...
void huffman::createMinHeap() {
    ...
    while (!inFile.eof()) {
        arr[id]->freq++;
        inFile.get(id); // id will automatically get updated
    }
    inFile.close();
...
// DONE with the help of GFG video
...
//DONE, do lookout in the docs for detailed explainataion with intuition :)7
void huffman::saveEncodedFile() {
    //Saving encoded (.huf) file
    inFile.open(inFileName, ios::in);   // reading from file
    outFile.open(outFileName, ios::out | ios::binary); // writing in the file in Binary
    string in = "";
    string s = "";
    char id;
    ...
    priority_queue <Node*, vector<Node*>, Compare> tempPQ(minHeap);  //A temporary priority queue tempPQ is created using the minHeap container. 
    // This is done to preserve the original minHeap and use it for other operations if needed.



    //Purpose: This code block is responsible for processing the characters and their associated codes in the
    // Huffman Tree to generate the metadata. Here's a breakdown of the steps:
    while (!tempPQ.empty()) {
        Node* curr = tempPQ.top();
        in += curr->data;   // current character
        ...
        //Saving decimal values of every 8-bit binary code //Encoding the metadata
        ...
        s.clear();  // Clearing the string 's' before the next iteration
    }
    s.clear();


    //Saving codes of every charachter appearing in the input file
    //Purpose: This code block is responsible for processing the input file data and converting it into the encoded form.
    ...
    while (!inFile.eof()) {
        ...
        while (s.length() > 8) {
            in += (char)binToDec(s.substr(0, 8));
            s = s.substr(8); // updates 's'  by extracting a substring starting from index 8 and assigns it back to 's'.
        }

        inFile.get(id);   // automatically gets the next file pointer
    }
...
//DONE intuitively, look over docs for detailed explaination.
void huffman::getTree() {
    ...
    for(int i = 0; i < size; i++) {
        ...
        inFile.read(&aCode, 1); // reads one byte from compressed file and stores it in 'aCode', representing the character data.
        ...
        hCodeStr = hCodeStr.substr(j+1); // getting the substring from after first occurence of 1, i.e getting the Huffman code of the character :)
